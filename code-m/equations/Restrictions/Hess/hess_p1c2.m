function [ h_penalidade_valor ] = hess_p1c2( x )
%PENALIDADE Summary of this function goes here
%   Detailed explanation goes here
% exemplo 1
        %st.: -2 -x1 -2x2 <=0
        %     8 - 6x1 +x1^2 -x2 <=0
        h_penalidade_valor(1,1) = 2*(- 6 +2*x(1))*(2*x(1)-6) + 4*(8 - 6*x(1) +x(1)^2 -x(2));        
        h_penalidade_valor(1,2) = -2*(2*x(1)-6);
        h_penalidade_valor(2,1) = -2*(2*x(1)-6);
        h_penalidade_valor(2,2) = 2;
% % exemplo 2
%         %st.: 2x1 +x2 -2 <=0
%         %     -x2 +1 <=0
